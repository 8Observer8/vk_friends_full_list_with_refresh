    ...
    QMainWindow(parent)
{
    ...
    // Online Radio
    allRadio = new QRadioButton(tr("All"));
    allRadio->setChecked(true);
    onlineRadio = new QRadioButton(tr("Online"));
    offlineRadio = new QRadioButton(tr("Offline"));
    ...
    connect(btnRefresh, SIGNAL(clicked()), this, SLOT(on_btnRefresh_clicked_slot()));
}

void MainWindow::on_btnRefresh_clicked_slot() {
    emit refresh();
}
...
void MainWindow::slotSuccess(QDomDocument dom) {
    ...
    if (allRadio->isChecked()) {
        ...
        onlineRadio_clicked_slot();
    } else {
        offlineRadio_clicked_slot();
    }
